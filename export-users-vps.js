...
/**
 * User Export Script for ClanPlug
 * 
 * USAGE:
 * 1. Upload this file to your VPS: /var/www/clanplug/backend/
 * 2. SSH into your VPS: ssh root@176.57.189.248
 * 3. Navigate to backend: cd /var/www/clanplug/backend
 * 4. Run: node export-users-vps.js
...
async function exportUsers() {
  try {
    ...
    // Save to JSON file
    const jsonFilename = path.join(exportDir, `users-export-${timestamp}.json`);
    fs.writeFileSync(jsonFilename, JSON.stringify(formattedUsers, null, 2));
    console.log(`✅ JSON export saved to: ${jsonFilename}`);

    // Save to CSV file
    const csvFilename = path.join(exportDir, `users-export-${timestamp}.csv`);
    const headers = [

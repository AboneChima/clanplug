...
export default function WithdrawalModal({ isOpen, onClose, balance, onSuccess }: WithdrawalModalProps) {
  ...
  // Calculate fees and net amount with 3% fee
  const calculateFees = (amount: number) => {
    if (!amount) return { fee: 0, netAmount: 0, totalDeduction: 0 };
    
    const feePercentage = 0.03; // 3% fee
    const fee = amount * feePercentage; // 3% of the withdrawal amount
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-500/20 to-pink-500/20 border border-red-500/30 flex items-center justify-center">
              <IoCashOutline className="w-5 h-5 text-red-600" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-gray-900">Withdraw NGN</h2>
        ...
        <div className="p-6 bg-gradient-to-r from-blue-50 to-purple-50 border-b border-gray-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-sm text-gray-600">Available NGN Balance</p>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              <IoCashOutline className="inline w-4 h-4 mr-1" />
              Amount (NGN)
            </label>
            <input
              type="number"
                ...
                <div className="text-sm space-y-1">
                  ...
                  <div className="mt-2 pt-2 border-t">
                    <div className="text-xs text-gray-600 text-center">
                      <span className="font-medium">Calculation:</span> {parseFloat(formData.amount).toFixed(2)} - {calculateFees(parseFloat(formData.amount)).fee.toFixed(2)} (3% fee) = ₦{calculateFees(parseFloat(formData.amount)).netAmount.toFixed(2)}
                    </div>
                  </div>
          ...
          <div className="bank-dropdown-container">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-10 text-gray-900 bg-white placeholder-gray-500"
                required
              />
              <IoChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />

...
export default function FollowingPage() {
  ...
  const loadFollowing = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/follow/${params.id}/following`, {
  ...
  const getUserInitials = (user: Following) => {
    if (user.firstName) {
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="sticky top-0 z-30 bg-black/95 backdrop-blur-xl border-b border-[#1a1a1a]">
        <div className="flex items-center gap-3 px-3 py-2.5">
          ...
          <h1 className="text-white text-sm font-semibold">Following</h1>
        </div>
      </div>

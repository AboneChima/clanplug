...
async function run() {
  try {
    ...
    await execCommand(`cat > /var/www/clanplug/backend/test-groups-api.js << 'EOF'
...
async function main() {
  try {
    ...
    const user = await prisma.user.findFirst({
      where: { email: 'deoraclee@gmail.com' }
    });
...
EOF`);
    ...
    console.log('\nRunning test...\n');
    await execCommand('cd /var/www/clanplug/backend && node test-groups-api.js');

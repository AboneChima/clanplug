...
/**
 * Simple Render Database Backup Script
 * 
 * This script exports your Render PostgreSQL database data
 * No PostgreSQL tools required!
 * 
 * Run with: node backup-render.js
 */

// Don't load .env file - we'll use command line or hardcoded Render URL
const { PrismaClient } = require('@prisma/client');
const fs = require('fs');
const path = require('path');

// Use environment variable or default to Render production database
const RENDER_DB_URL = 'postgresql://lordmoon:LzJr3MUNrSoX4eb7k2D9eKviJpQTMXOm@dpg-d4b12124d50c73cv58bg-a.oregon-postgres.render.com/lordmoon';
const databaseUrl = process.env.RENDER_DATABASE_URL || RENDER_DB_URL;

console.log('🔗 Connecting to Render database...');
console.log('🌐 Host:', databaseUrl.split('@')[1]?.split('/')[0] || 'Render PostgreSQL');
...
async function exportDatabase() {
  try {
    ...
    try {
      backup.data.marketplaceListings = await prisma.listing.findMany({
        include: {
          seller: true
        }
      });
      ...
      console.log('   ⚠️ Marketplace listings table not found, skipping...');
      backup.data.marketplaceListings = [];
    }
